...
specified otherwise, the FFTs are calculated after padding each binary grain map/image to its nearest square. The real (from binary data) and complex (from grayscale data) FFTs are returned in grid format within the 'grains.prop.fftReal' and 'grains.prop.fftComplex' structure variables.
