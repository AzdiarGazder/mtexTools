...
%% OBJECTIVE:
% The objective of this script is to spatially rotate the ebsd map WITHOUT
% disturbing the ebsd map orientation data. The user must specify the
% spatial rotations needed to enable Mtex to display the EBSD map data
% in a spatial format that exactly matches OI Channel-5 or Aztec
% Crystal.
%
...
% Step 2: spatially rotate the ebsd map WITHOUT disturbing the ebsd map
% orientation data. The user must specify the spatial rotations needed to
% enable Mtex to display the EBSD map data in a spatial format that exactly
% matches OI Channel-5 or Aztec Crystal.
...
for ii = 1:length(phi1)
    for jj = 1:length(Phi)
        for kk = 1:length(phi2)
            ...
            % Step 2: rotate the spatial data to CORRECT the view of the
            % ebsd map
